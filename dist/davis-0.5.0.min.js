...
Davis.history=function(){var a=[],b=!0,c=function(a){return function(c){c.state?(c=c.state,c.__proto__=Davis.Request.prototype,a(c)):b||a(Davis.Request.forPageLoad());b=!1}};return{onChange:function(b){a.push(b);b=c(b);window.addEventListener("popstate",b,!0)},current:function(){return window.location.pathname},assign:function(c){history.pushState(c,c.title,c.location());Davis.utils.forEach(a,function(a){a(c)})},replace:function(c){history.replaceState(c,c.title,c.location());Davis.utils.forEach(a,
function(a){a(c)})}}}();Davis.location=function(){var a=Davis.history;return{setLocationDelegate:function(b){a=b},current:function(){return a.current()},assign:function(b){a.assign(b)},replace:function(b){a.replace(b)},onChange:function(b){a.onChange(b)}}}();
Davis.Request=function(a){var b=this;this.params={};this.title=a.title;this.queryString=a.fullPath.split("?")[1];this._staleCallback=function(){};this.queryString&&Davis.utils.forEach(this.queryString.split("&"),function(a){var f=a.split("=")[0];a=a.split("=")[1];var d;if(d=/^(\w+)\[(\w+)\]/.exec(f)){var e=d[1];f=d[2];d=b.params[e]||{};d[f]=a;b.params[e]=d}else b.params[f]=a});this.method=(this.params._method||a.method).toLowerCase();this.path=a.fullPath.replace(/\?.+$/,"");this.delegateToServer=
...
Davis.Request.prototype.location=function(){return this.method==="get"?this.path:""};Davis.Request.prototype.toString=function(){return[this.method.toUpperCase(),this.path].join(" ")};Davis.Request.forPageLoad=function(){return new this({method:"get",fullPath:Davis.location.current(),title:document.title,forPageLoad:!0})};Davis.Request.prev=null;
Davis.App=function(){var a=0,b=function(){this.id=[(new Date).valueOf(),a++].join("-");this.boundToInternalEvents=this.running=!1};b.prototype=jQuery.extend({configure:function(a){a.call(this.settings)},use:function(a){a.apply(this,Davis.utils.toArray(arguments,1))},helpers:function(a){for(property in a)a.hasOwnProperty(property)&&(Davis.Request.prototype[property]=a[property])},settings:{linkSelector:"a",formSelector:"form",logger:Davis.logger,throwErrors:!0,handleRouteNotFound:!1,generateRequestOnPageLoad:!0},
start:function(){var a=this;if(Davis.supported()){var b=function(a){return function(b){b=b.run(a,a);return typeof b==="undefined"||b}},d=function(d){if(Davis.utils.every(a.lookupBeforeFilter(d.method,d.path),b(d))){a.trigger("lookupRoute",d);var e=a.lookupRoute(d.method,d.path);if(e){a.trigger("runRoute",d,e);try{e.run(d),a.trigger("routeComplete",d,e)}catch(h){a.trigger("routeError",d,e,h)}Davis.utils.every(a.lookupAfterFilter(d.method,d.path),b(d))}else a.trigger("routeNotFound",d)}else a.trigger("requestHalted",
d)},e;this.boundToInternalEvents||function(){a.bind("runRoute",function(b){a.settings.logger.info("runRoute: "+b.toString())}).bind("routeNotFound",function(b){!a.settings.handleRouteNotFound&&!b.isForPageLoad&&(a.stop(),b.delegateToServer());a.settings.logger.warn("routeNotFound: "+b.toString())}).bind("start",function(){a.settings.logger.info("application started")}).bind("stop",function(){a.settings.logger.info("application stopped")}).bind("routeError",function(b,d,e){if(a.settings.throwErrors)throw e;
a.settings.logger.error(e.message,e.stack)});Davis.location.onChange(function(a){d(a)});a.boundToInternalEvents=!0}();this.listen();this.trigger("start");this.running=!0;this.settings.generateRequestOnPageLoad&&d(Davis.Request.forPageLoad())}else this.trigger("unsupported")},stop:function(){this.unlisten();this.trigger("stop");this.running=!1}},Davis.listener,Davis.event);Davis.router.call(b.prototype);return b}();
