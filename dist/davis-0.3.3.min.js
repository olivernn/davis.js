...
Davis.listener=function(){var c=function(b){return function(){var e=new Davis.Request(b.call($(this)));Davis.history.pushState(e);return false}},d=c(function(){return{method:"get",fullPath:this.attr("href"),title:this.attr("title")}}),a=c(function(){return{method:this.attr("method"),fullPath:[this.attr("action"),function(b){return b.serializeArray().map(function(e){return[e.name,e.value].join("=")}).join("&")}(this)].join("?"),title:this.attr("title")}});return{listen:function(){$(document).delegate(this.settings.formSelector,
...
function(){if(arguments.length==1)var b=/.+/,e=arguments[0];else if(arguments.length==2){b=arguments[0];e=arguments[1]}c._filterCollection[d].push(new Davis.Route(/.+/,b,e))};var a="lookup"+d.replace(/^\w/,function(b){return b.toUpperCase()})+"Filter";c[a]=function(b,e){return c._filterCollection[d].filter(function(f){return f.match(b,e)})}});c._routeCollection=[];c._filterCollection={before:[],after:[]};c.lookupRoute=function(d,a){return this._routeCollection.filter(function(b){return b.match(d,
...
Davis.Request=function(c){var d=this;this.params={};this.title=c.title;this.queryString=c.fullPath.split("?")[1];this._staleCallback=function(){};this.queryString&&this.queryString.split("&").forEach(function(a){var b=a.split("=")[0];a=a.split("=")[1];var e;if(e=/^(\w+)\[(\w+)\]/.exec(b)){var f=e[1];b=e[2];e=d.params[f]||{};e[b]=a;d.params[f]=e}else d.params[b]=a});this.method=this.params._method||c.method;this.path=c.fullPath.replace(/\?.+$/,"");Davis.Request.prev&&Davis.Request.prev.makeStale(this);
...
Davis.App=function(){var c=0,d=function(){this.id=[(new Date).valueOf(),c++].join("-");this.running=false};d.prototype=$.extend({configure:function(a){a.call(this.settings)},use:function(a){a.apply(this,Array.prototype.slice.call(arguments,1))},helpers:function(a){for(property in a)if(a.hasOwnProperty(property))Davis.Request.prototype[property]=a[property]},settings:{linkSelector:"a",formSelector:"form",logger:Davis.logger,throwErrors:true,generateRequestOnPageLoad:true},start:function(){var a=this;
if(Davis.supported()){var b=function(f){return function(g){g=g.run(f,f);return typeof g==="undefined"||g}},e=function(f){if(a.lookupBeforeFilter(f.method,f.path).every(b(f))){a.trigger("lookupRoute",f);var g=a.lookupRoute(f.method,f.path);if(g){a.trigger("runRoute",f,g);try{g.run(f);a.trigger("routeComplete",f,g)}catch(h){a.trigger("routeError",f,g,h);a.settings.logger.error(h.message,h.stack);if(a.settings.throwErrors)throw h;}a.lookupAfterFilter(f.method,f.path).every(b(f))}else a.trigger("routeNotFound",
f)}else a.trigger("requestHalted",f)};Davis.history.onChange(function(f){e(f)});this.bind("runRoute",function(f){a.settings.logger.info("runRoute: "+f.toString())}).bind("routeNotFound",function(f){a.settings.logger.warn("routeNotFound: "+f.toString())}).bind("start",function(){a.settings.logger.info("application started")});this.listen();this.trigger("start");this.running=true;this.settings.generateRequestOnPageLoad&&e(Davis.Request.forPageLoad())}else this.trigger("unsupported")},stop:function(){this.unlisten();
this.trigger("stop");this.running=false}},Davis.listener,Davis.event);Davis.router.call(d.prototype);return d}();
