...
a)})[0]}};Davis.history=function(){var c=[],d=function(a){return function(b){if(b.state){b=b.state;b.__proto__=Davis.Request.prototype;a(b)}else a(Davis.Request.forPageLoad())}};return{replaceState:function(a){history.replaceState(a,a.title,a.location());c.forEach(function(b){b(a)})},pushState:function(a){history.pushState(a,a.title,a.location());c.forEach(function(b){b(a)})},onChange:function(a){c.push(a);a=d(a);window.addEventListener("popstate",a,true)}}}();
...
Davis.App=function(){var c=0,d=function(){this.id=[(new Date).valueOf(),c++].join("-");this.running=false};d.prototype=$.extend({configure:function(a){a.call(this.settings)},use:function(a){a.apply(this,Array.prototype.slice.call(arguments,1))},helpers:function(a){for(property in a)if(a.hasOwnProperty(property))Davis.Request.prototype[property]=a[property]},settings:{linkSelector:"a",formSelector:"form",logger:Davis.logger,throwErrors:true},start:function(){var a=this;if(Davis.supported()){var b=
function(e){return function(g){g=g.run(e,e);return typeof g==="undefined"||g}},f=function(e){if(a.lookupBeforeFilter(e.method,e.path).every(b(e))){a.trigger("lookupRoute",e);var g=a.lookupRoute(e.method,e.path);if(g){a.trigger("runRoute",e,g);try{g.run(e);a.trigger("routeComplete",e,g)}catch(h){a.trigger("routeError",e,g,h);a.settings.logger.error(h.message,h.stack);if(a.settings.throwErrors)throw h;}a.lookupAfterFilter(e.method,e.path).every(b(e))}else a.trigger("routeNotFound",e)}else a.trigger("requestHalted",
e)};Davis.history.onChange(function(e){f(e)});this.bind("runRoute",function(e){a.settings.logger.info("runRoute: "+e.toString())}).bind("routeNotFound",function(e){a.settings.logger.warn("routeNotFound: "+e.toString())}).bind("start",function(){a.settings.logger.info("application started")});this.listen();this.trigger("start");this.running=true;f(Davis.Request.forPageLoad())}else this.trigger("unsupported")},stop:function(){this.unlisten();this.trigger("stop");this.running=false}},Davis.listener,
Davis.event);Davis.router.call(d.prototype);return d}();
