...
Davis.logger=function(){var a=function(d){d=Davis.utils.toArray(d);d.unshift("["+Date()+"]");return d.join(" ")};return{error:function(){window.console&&console.error(a(arguments))},info:function(){window.console&&console.info(a(arguments))},warn:function(){window.console&&console.warn(a(arguments))}}}();
Davis.Route=function(){var a=/:([\w\d]+)/g,d=function(b,c,e){this.paramNames=function(){for(var g=[],f;f=a.exec(c);)g.push(f[1]);return g}();this.path=function(){if(c instanceof RegExp)return c;else{var g=c.replace(a,"([^/]+)");c.lastIndex=0;return RegExp("^"+g+"$","gi")}}();this.method=b instanceof RegExp?b:RegExp("^"+b+"$","i");this.callback=e};d.prototype={match:function(b,c){this.reset();return this.method.test(b)&&this.path.test(c)},reset:function(){this.method.lastIndex=0;this.path.lastIndex=
0},run:function(b){this.reset();var c=this.path.exec(b.path);if(c){c.shift();for(var e=0;e<c.length;e++)b.params[this.paramNames[e]]=c[e]}return this.callback.call(b,b)},toString:function(){return[this.method,this.path].join(" ")}};return d}();
